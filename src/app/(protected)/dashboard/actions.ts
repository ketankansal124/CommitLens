...
export async function askQuestion(question: string, projectId: string) {
    ...
    SELECT "fileName", "sourceCode", "summary",
           1 - ("summaryEmbedding" <=> CAST(${vectorQuery} AS vector)) AS similarity
    FROM "SourceCodeEmbedding"
    WHERE ("summaryEmbedding" <=> CAST(${vectorQuery} AS vector)) < 0.5
    AND "projectId" = CAST(${projectId} AS TEXT)
    ...
    (async () => {
        const { textStream } = await streamText({
            ...
            prompt: `
      You are an AI code assistant who answers questions about the codebase. Your target audience is a technical intern who needs guidance.
